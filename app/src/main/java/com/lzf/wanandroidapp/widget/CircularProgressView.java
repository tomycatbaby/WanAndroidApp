...
public class CircularProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mRectF, 270, 360 * mProgress / 1000, false, mProgPaint);
    }
    ...
    public void setProgress(int progress, long animTime) {
        if (animTime <= 0) setProgress(progress);
            ...
            Log.d("lzf", "setProgress onAnimationUpdate: " + mProgress + " progress:" + progress);
            ValueAnimator animator = ValueAnimator.ofInt(mProgress, progress);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    Log.d("lzf", "onAnimationUpdate: " + animation.getAnimatedValue());
